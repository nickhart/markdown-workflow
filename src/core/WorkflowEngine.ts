...
import { WorkflowFileSchema, type WorkflowFile, type CollectionMetadata, type ProjectConfig } from './schemas.js';
...
export class WorkflowEngine {
  ...
  async getCollections(workflowName: string): Promise<Collection[]> {
    ...
    const collectionDirs = fs.readdirSync(workflowCollectionsDir, { withFileTypes: true })
  ...
  private async executeNotesAction(
    ...
    const templatePath = path.join(this.systemRoot, 'workflows', workflow.workflow.name, notesTemplate.file);
