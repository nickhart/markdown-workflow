...
const safeString = (maxLength: number) =>
  z
    ...
    .refine((str) => str.length > 0, 'Cannot be empty after sanitization')
    .refine((str) => !/^\s*$/.test(str), 'Cannot contain only whitespace');
  ...
  .refine((content) => {
    ...
    if (mermaidBlocks) {
      for (const block of mermaidBlocks) {
        const diagram = block.replace(/```mermaid\s*|\s*```/g, '').trim();
...
function isValidMermaidSyntax(diagram: string): boolean {
  ...
  return validDiagramTypes.some((type) => firstLine.startsWith(type.toLowerCase()));
}
...
const collectionIdSchema = z
  ...
  .max(
    ...
    `Collection ID cannot exceed ${MAX_COLLECTION_ID_LENGTH} characters`,
  )
  .regex(
    /^[a-z0-9_-]+$/,
...
export const getTemplatesSchema = z
  .object({
...
export const formatPresentationSchema = z
  .object({
...
export const downloadPresentationSchema = z
  .object({
