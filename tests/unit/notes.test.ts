...
workflows: {}`,
  ...
  it('should create notes for a collection', async () => {
    const options = { cwd: '/mock/project', configDiscovery };

    await expect(
      notesCommand('job', 'test_company_developer_20250122', 'recruiter', options),
  ...
  it('should handle missing collection', async () => {
    ...
    await expect(
      notesCommand('job', 'nonexistent_collection', 'recruiter', options),
    ).rejects.toThrow();
  });
  ...
  it('should handle missing workflow', async () => {
    ...
    await expect(
      notesCommand('nonexistent', 'test_company_developer_20250122', 'recruiter', options),
    ).rejects.toThrow();
  });
});
...
workflows: {}`,
  ...
  it('should list available note types for a workflow', async () => {
    const options = { cwd: '/mock/project', configDiscovery };

    await expect(listNoteTypesCommand('job', options)).rejects.toThrow('System root not found');
  });

  it('should handle missing workflow', async () => {
    const options = { cwd: '/mock/project', configDiscovery };

    await expect(listNoteTypesCommand('nonexistent', options)).rejects.toThrow();
  });
});
