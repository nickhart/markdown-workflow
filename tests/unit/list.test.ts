...
workflows: {}`,
    ...
    const collectionsPath = `${projectPath}/collections`;
    mockSystemInterface.addMockDirectory(collectionsPath);
    mockSystemInterface.addMockDirectory(`${collectionsPath}/job`);

    const collectionPath = `${collectionsPath}/job/test_company_developer_20250122`;
  ...
  it('should list collections for a workflow', async () => {
    ...
    await expect(listCommand('job', options)).rejects.toThrow('System root not found');
  });
  ...
  it('should filter by status when provided', async () => {
    const options = { cwd: '/mock/project', configDiscovery, status: 'active' };

    await expect(listCommand('job', options)).rejects.toThrow('System root not found');
  });

  it('should handle different output formats', async () => {
    const options = { cwd: '/mock/project', configDiscovery, format: 'json' as const };
